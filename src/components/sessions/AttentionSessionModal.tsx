...
export function AttentionSessionModal({
  ...
  // =========================================================================
  // CÁLCULOS DE OBJETIVOS DEL CICLO ACTUAL (DESVINCULADOS DE CICLOS ANTERIORES)
  // =========================================================================
  ...
  // =========================================================================
  // Condición estricta: El botón para completar el objetivo general SOLO se habilita cuando
  // existen objetivos secundarios completados en ESTE ciclo (completedObjectives.length > 0)
  // y NO existen objetivos secundarios pendientes (pendingObjectives.length === 0)
  ...
  const handleSaveEditedPatient = async (updatedPatient: Patient) => {
    await storageService.updatePatient(updatedPatient)
    if (onPatientUpdated) {
      onPatientUpdated(updatedPatient)
    }
      ...
      >
        <div className="space-y-6 pb-4">
          ...
          <div className="bg-white rounded-2xl border border-zinc-200/90 shadow-2xs overflow-hidden transition-all duration-200">
            <div className="px-5 py-3.5 bg-zinc-100/70 border-b border-zinc-200/80 flex items-center justify-between">
              <div
              ...
              <div className="flex items-center gap-3">
                ...
                <Button
                  ...
                  className="h-7 px-2.5 text-xs gap-1 font-semibold text-zinc-700 hover:text-zinc-900 border-zinc-300 bg-white"
                >
                ...
                <button
                  type="button"
                  onClick={() => setShowFullProfile(!showFullProfile)}
                  className="flex items-center gap-1 text-xs text-zinc-500 hover:text-zinc-800 font-medium"
                >
                  <span>{showFullProfile ? 'Ocultar' : 'Mostrar'}</span>
                  {showFullProfile ? (
          ...
          <div className="bg-white rounded-2xl border border-zinc-200 shadow-sm p-5 space-y-4">
            <div className="flex flex-wrap items-center justify-between gap-3 pb-3 border-b border-zinc-100">
              ...
              {/* Botón único y limpio en la parte superior cuando no hay objetivo activo */}
            ...
            {patient.objetivoGeneral && patient.objetivoGeneral.trim() ? (
              <div className="bg-gradient-to-r from-lime-50/90 via-lime-50/50 to-white p-4 rounded-xl border border-lime-200/90 flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div className="space-y-1 flex-1">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs text-zinc-500 font-medium">
                      {completedObjectives.length} de {completedObjectives.length + pendingObjectives.length} objetivos secundarios logrados en este ciclo
                    </span>
                  </div>
                ...
                <div className="flex flex-col items-end gap-1.5 shrink-0">
                  <Button
                    ...
                    title={
                      canCompleteGeneralObjective
                        ? 'Todos los objetivos secundarios de este ciclo están logrados. Clic para completar objetivo padre.'
                        : completedObjectives.length === 0
                        ? 'Debes registrar y lograr objetivos secundarios en las sesiones de este ciclo antes de completar.'
                  ...
                  {!canCompleteGeneralObjective && (
                    <span className="text-[10px] text-amber-700 font-medium flex items-center gap-1">
                      ...
                      {pendingObjectives.length > 0
                        ...
                        : completedObjectives.length === 0
                        ? 'Registra atenciones y objetivos para este nuevo ciclo'
              ...
              !isCreatingNewGenObj && (
                <div className="p-4 rounded-xl bg-zinc-50 border border-dashed border-zinc-200 text-xs text-zinc-500 flex items-center gap-2.5">
          ...
          {activeTab === 'objectives_summary' && (
            <div className="bg-white rounded-2xl border border-zinc-200 p-6 space-y-6 animate-in fade-in duration-150">
              ...
              {/* Métricas de Logro del Paciente en este ciclo */}
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                ...
                <div className="bg-lime-50/70 p-4 rounded-xl border border-lime-200">
                  ...
                  <p className="text-[11px] text-lime-700/80 mt-0.5">Completados en este ciclo</p>
                </div>
              ...
              <div className="space-y-3 pt-2">
                ...
                {completedObjectives.length === 0 ? (
                  <div className="p-4 rounded-xl bg-zinc-50 border border-dashed border-zinc-200 text-center text-xs text-zinc-400">
                    Aún no hay objetivos marcados como Logrado en este ciclo.
                  </div>
                ) : (
              ...
              <div className="space-y-3 pt-4">
                ...
                {pendingObjectives.length === 0 ? (
                  <div className="p-4 rounded-xl bg-zinc-50 border border-dashed border-zinc-200 text-center text-xs text-zinc-400">
                    No hay objetivos secundarios pendientes en este ciclo.
                  </div>
                ) : (
                  <div className="space-y-2">
